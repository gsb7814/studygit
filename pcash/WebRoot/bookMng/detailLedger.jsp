...
<s:form name="frm" action="detailLedger.action" method="post">  
 ...
 <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#DDECFF">
  ...
  <tr>
    ...
    <td>当前位置： <a href="../index.htm" target="_top">首页</a> &gt;&gt; 帐薄管理 &gt;&gt; 明细分类账</td>
  </tr>
  <tr>
...
<table width="780" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#999999">
  ...
  <tr>
	...
	headerKey="0" headerValue="请选择" value="%{#unitCode.trim()}"></s:select>
          ...
          -  <input   type="text"   name="voucherDateend" value="<s:property value="#session.tvoucherDateend"/>" >  
                    &nbsp;&nbsp;<strong>会计科目：</strong> 
        ...
        <td width="71" height="25"><div align="center">
         <s:submit name="submit" value="生成" method="search"></s:submit>
        </div></td>
      </tr>
    </table></td>
    ...
    <%
  List hj = (List)ActionContext.getContext().get("sumhjdetail");
   if (hj != null)
  {
 ...
 %> 
  ...
  List lj = (List)ActionContext.getContext().get("sumljdetail");
   if (lj != null)
  {
  ...
  <tr>
    ...
    <% 
  PaginationVO lst = (PaginationVO)ActionContext.getContext().get("voucherSum");
   if (lst != null)
  {
 ...
 <s:url id="url_first" value="detailLedgerfy.action">
         <s:param name="CP" value="0"></s:param>
         
  </s:url>
  
  <s:if test="CP gt 0">
  <s:url id="url_pre" value="detailLedgerfy.action">
...
<s:set name="tp"><%=totalpage-1%></s:set>
 ...
 <s:if test="CP lt #tp">
     <s:url id="url_next" value="detailLedgerfy.action">
         <s:param name="CP" value="CP+1"></s:param>
     </s:url> 
</s:if>

    
     <s:url id="url_last" value="detailLedgerfy.action">
...
<s:a href="%{url_first}"> 首页 </s:a> <s:a href="%{url_pre}">上一页</s:a> <s:a href="%{url_next}">下一页</s:a>  <s:a href="%{url_last}">尾页</s:a></td>
  ...
  <tr>
    <td height="25" colspan="7" align="left" bgcolor="#FFFFFF" ><div align="right">
      <s:submit name="export" value="导出"   method="exportDetailLedger"/> 
      </div></td>
  </tr>
</table>
